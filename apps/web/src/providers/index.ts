 ...
 * Route/Notification/Storage는 애초에 실제 구현체가 없어 `local/`의 단순
 * 구현체가 지금의 유일한 구현이다 (이름은 Mock이 아니라 Local — 조건부로
 * 켜고 끄는 대체품이 아니라 현재 서비스가 실제로 쓰는 구현체라서).
 */
...
export function getStorageProvider(): StorageProvider {
  if (!storageSingleton) storageSingleton = new LocalStorageProvider();
